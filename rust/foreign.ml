(** The synchronous target slice consumes checked, pinned constant metadata.
    Schema instantiation and async effects require the later crate printer. *)
open Kanon_kernel
...
let foreign_call entries (row : Rir.foreign) =
  ...
  let* () = if List.is_empty row.effects then Ok ()
    else refuse ("effectful foreign call " ^ row.name) in
  ...
  Ok (params, result, fun arguments -> Template.call ~parameters ~types:[] ~arguments entry.print_rule)
