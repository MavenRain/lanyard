...
let runtime model_errors effect = {|// Generated by lanyard's native Rust printer.
use std::sync::Arc;
...
impl std::error::Error for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::Digit | Self::Arithmetic|} ^ (if model_errors then " | Self::ModelRange" else "") ^ {| => None,
...
let native ?(model_errors = false) rows =
  ...
  if List.length (List.sort_uniq String.compare names) <> List.length names then invalid "duplicate function"
    ...
    Ok (runtime model_errors effect ^ family_code ^ String.concat "\n" (List.map declaration types) ^ "\n"
