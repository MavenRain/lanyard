...
let runtime ?(uri_errors = false) model_errors text_errors effect = {|// Generated by lanyard's native Rust printer.
use std::sync::Arc;
...
impl std::error::Error for Error {
  ...
  ^ (if uri_errors then " | Self::InvalidUri" else "") ^ {| => None,
...
let entry_point output signatures name =
  ...
  if not (List.is_empty signature.params) then invalid ("entry point requires runtime arguments: " ^ name)
    ...
    Ok (helper ^ "\n" ^ attribute ^ Effects.prefix signature.effect ^ "fn main() -> Result<(), "
      ^ error ^ "> {\n    " ^ body ^ "\n    Ok(())\n}\n")
let native ?entrypoint ?(entry_output = Discard) ?(model_errors = false) ?(text_errors = false) ?(uri_errors = false) rows =
  ...
  if List.length (List.sort_uniq String.compare names) <> List.length names then invalid "duplicate function"
    ...
    Ok (runtime ~uri_errors model_errors text_errors effect ^ family_code ^ String.concat "\n" (List.map declaration types) ^ "\n"
