...
let closure_signature signatures fid arity =
  ...
  let* () = match signature.effect with Effects.Sync -> Ok ()
    | Effects.Async_db -> refuse ("async closure " ^ name) in
...
let rec expression families signatures env term =
  ...
  | RCall (name, terms) ->
      ...
      let action = Option.fold ~none:(fun () -> primitive name args)
        ~some:(fun signature () ->
          ...
          Ok { code = function_name name ^ "(" ^ String.concat ", " args ^ ")"
            ^ Effects.await signature.effect ^ "?"; ty = signature.result })
  ...
  | RForeign (row, terms) ->
      let* params, result, render, _effect = Target.foreign_call row in
...
let runtime ?(uri_errors = false) ?(form_errors = false) model_errors text_errors effect = {|// Generated by lanyard's native Rust printer.
use std::sync::Arc;

#[derive(Clone, Debug, PartialEq, Eq)]
struct Nat(Vec<u8>);

|} ^ (match effect with Effects.Sync -> {|#[derive(Debug)]
enum Error { Digit, Arithmetic|} ^ (if model_errors then ", ModelRange" else "")
  ^ (if text_errors then ", ModelByteRange, ModelUtf8" else "")
  ^ (if uri_errors then ", InvalidUri" else "")
  ^ (if form_errors then ", InvalidForm" else "") ^ {| }
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Digit => f.write_str("invalid natural digit"),
            Self::Arithmetic => f.write_str("natural limb invariant failed"),
|} ^ (if model_errors then "            Self::ModelRange => f.write_str(\"model Nat is outside 0..=i64::MAX\"),\n" else "")
  ^ (if text_errors then "            Self::ModelByteRange => f.write_str(\"model text element is outside 0..=255\"),\n"
  ^ "            Self::ModelUtf8 => f.write_str(\"model text is not valid UTF-8\"),\n" else "")
  ^ (if uri_errors then "            Self::InvalidUri => f.write_str(\"invalid origin-form URI\"),\n" else "")
  ^ (if form_errors then "            Self::InvalidForm => f.write_str(\"invalid form or missing field\"),\n" else "") ^ {|        }
    }
}
impl std::error::Error for Error {}
|} | Effects.Async_db -> {|#[derive(Debug)]
enum Error { Digit, Arithmetic, Database(toasty::Error)|} ^ (if model_errors then ", ModelRange" else "")
  ^ (if text_errors then ", ModelByteRange, ModelUtf8" else "")
  ^ (if uri_errors then ", InvalidUri" else "")
  ^ (if form_errors then ", InvalidForm" else "") ^ {| }
impl std::fmt::Display for Error {
...
impl std::error::Error for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::Digit | Self::Arithmetic|} ^ (if model_errors then " | Self::ModelRange" else "")
  ^ (if text_errors then " | Self::ModelByteRange | Self::ModelUtf8" else "")
  ^ (if uri_errors then " | Self::InvalidUri" else "")
  ^ (if form_errors then " | Self::InvalidForm" else "") ^ {| => None,
  ...
  if not (List.is_empty signature.params) then invalid ("entry point requires runtime arguments: " ^ name)
    ...
    let* error, body, helper = match output with
      ...
      | Print_model (layout, printer) ->
          ...
          if not (same expected signature.result) then invalid "entry point result differs from printed model" else
          let call, helper = match signature.effect with
            | Effects.Sync -> call, ""
            | Effects.Async_db -> "lan_require_send(" ^ call ^ ")",
                "\nfn lan_require_send<F: std::future::Future + Send>(future: F) -> F { future }\n" in
          Ok ("LanMainError", "let value = " ^ call ^ Effects.await signature.effect
            ^ "?;\n    " ^ printer ^ "(&value)?;", helper) in
    Ok (helper ^ "\n" ^ attribute ^ Effects.prefix signature.effect ^ "fn main() -> Result<(), "
      ^ error ^ "> {\n    " ^ body ^ "\n    Ok(())\n}\n")
let native ?entrypoint ?(entry_output = Discard) ?(model_errors = false) ?(text_errors = false) ?(uri_errors = false) ?(form_errors = false) rows =
  ...
  let* signatures = rows |> List.concat_map (fun (_name, entry) ->
    ...
    | Erase.Code decls -> List.filter_map (function
      ...
      | RFun (fid, params, result, body) -> Some (
          let* params = all (List.map repr params) in
          let* result = repr result in Ok { name = fid_text fid; params; result; body; effect = Effects.Sync })) decls)
    |> all in
  ...
  if List.length (List.sort_uniq String.compare names) <> List.length names then invalid "duplicate function"
    ...
    let* entry = Option.fold ~none:(Ok "") ~some:(entry_point entry_output signatures) entrypoint in
    ...
    let* bodies = all (List.map (fun signature ->
      ...
      Ok (Effects.prefix signature.effect ^ "fn " ^ function_name signature.name ^ "(" ^ String.concat ", " params ^ ") -> Result<"
    ...
    Ok (runtime ~uri_errors ~form_errors model_errors text_errors effect ^ family_code ^ String.concat "\n" (List.map declaration types) ^ "\n"
