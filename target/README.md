...
The target catalog provides 27 proposed foreign declarations: nine type
constants and eighteen constant or schema declarations. M1 adds
`Model.delete_by_id`, `Model.update`, `Model.all`, `Text.trim`,
`Text.is_empty`, `Uri.from_text`, `Form.field`, `Response.text`,
`Html.text`, `Response.html`, `Text.concat` and `Text.from_nat`
...
The generated module has 176 lines after adding text concatenation.
...
the additional 72 measured lines exceed that proposed allowance, which
