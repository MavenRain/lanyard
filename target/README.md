...
The target catalog provides 43 proposed foreign declarations: nine type
constants and thirty-four constant or schema declarations. M1 adds
...
`Text.equal`, `Text.length`, `Text.contains`, `Text.starts_with`, `Text.ends_with`, `Text.replace`,
`Text.repeat`, `Text.trim_start`, `Text.trim_end`, `Text.to_ascii_lowercase`,
`Text.to_ascii_uppercase`, `Uri.to_text`, `Uri.path`, `Uri.query` and
...
The generated module has 260 lines after adding the directional trimming adapters.
...
the additional 156 measured lines exceed that proposed allowance, which
