...
{"name":"Model.delete_by_id","type":"(0 M : Type 0) -> (0 Key : Type 0) -> (key : Key) -> (db : Db) -> prod ()","quantities":["0","0","w","w"],"effects":["DbExec","toasty::Error"],"print":"#{M}::delete_by_id(&mut #{db}, #{key}).await?","kind":"schema","status":"PROPOSED"}
{"name":"Model.update","type":"(0 M : Type 0) -> (fields : M) -> (db : Db) -> M","quantities":["0","w","w"],"effects":["DbExec","toasty::Error"],"print":"{ let __lan_fields = #{M} { #{fields} }; let mut __lan_row = #{M}::get_by_id(&mut #{db}, &__lan_fields.id).await?; toasty::update!(__lan_row { #{fields} }).exec(&mut #{db}).await?; __lan_row }","kind":"schema","status":"PROPOSED"}
