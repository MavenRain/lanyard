"""Check prefix and suffix matching, validation, evaluation and native agreement."""
import argparse
import contextlib
import itertools
...
def matches(text, needle):
    return text.startswith(needle) if OPERATION == "starts_with" else text.endswith(needle)


def mutation_cases():
    call = f"__lan_text.{OPERATION}(__lan_needle.as_str())"
    opposite = "ends_with" if OPERATION == "starts_with" else "starts_with"
    return [
        ("substring", call, "__lan_text.contains(__lan_needle.as_str())"),
        ("opposite", call, f"__lan_text.{opposite}(__lan_needle.as_str())"),
        ("reversed", call, f"__lan_needle.{OPERATION}(__lan_text.as_str())"),
        ("nonempty", call, f"!__lan_needle.is_empty() && {call}"),
...
# Every subprocess this file starts gets CALL_BUDGET seconds. The relaunch in
# __main__ waits for one child under CHILD_BUDGET, a ceiling and not a sum: the
# --mutations child runs COMPILES compile rounds (clean, the five mutants and
# restored), each a git init, one gateledger compile and one binary run, and the
# plain child runs the unittest suite (many short driver calls plus one compile).
# A child that stays under CALL_BUDGET on every call can still be killed at CHILD_BUDGET.
CALL_BUDGET = 180
COMPILES = 7
CHILD_BUDGET = COMPILES * CALL_BUDGET + 2 * CALL_BUDGET


def run(*args, cwd=ROOT):
    return subprocess.run([str(arg) for arg in args], cwd=cwd, capture_output=True,
                          timeout=CALL_BUDGET)


def require(result):
    if result.returncode != 0 or result.stderr:
        sys.exit(f"LAN-TEXT-BOUNDARIES FAIL: {result.stdout!r} {result.stderr!r}")
...
def native_harness():
    pairs = ",\n".join(f"({rust_string(left)}, {rust_string(right)}, "
                       f"{rust_string('yes' if matches(left, right) else 'no')})" for left, right in PAIRS)
...
def compile_native(work, emitted, name):
    ...
    if result.returncode != 0:
        sys.exit(f"LAN-TEXT-BOUNDARIES FAIL native build: {result.stdout!r} {result.stderr!r}")
    return run(binary)
...
class TextBoundaries(unittest.TestCase):
    def setUp(self):
        ...
        directory = tempfile.TemporaryDirectory(prefix="text-boundaries-", dir=ROOT / ".gatework")
        self.addCleanup(directory.cleanup)
        self.work = Path(directory.name)

    def test_form_branching(self):
        program = self.work / "form.lan"
        program.write_text((ROOT / "test/fixtures/text-contains.lan").read_text()
                           .replace("Text.contains", f"Text.{OPERATION}"))
        for action in ["save", "Save", "save ", "", "saved", "save\0", "保存"]:
            with self.subTest(action=action):
                ...
                self.assertEqual((actual.returncode, actual.stdout, actual.stderr),
                                 (0, response("found" if matches(action, "save") else "not found"), b""))

    def test_missing_form_field(self):
        program = self.work / "missing.lan"
        program.write_text((ROOT / "test/fixtures/text-contains.lan").read_text()
                           .replace("Text.contains", f"Text.{OPERATION}"))
        actual = run(DRIVER, "run", "--request", "/", "--form", "", program)
    ...
    def test_interpreter_pairs(self):
        ...
        for left, right in PAIRS:
            with self.subTest(left=left, right=right):
                ...
                self.assertEqual((actual.returncode, actual.stdout, actual.stderr),
                                 (0, response("yes" if matches(left, right) else "no"), b""))
    ...
    def test_effectful_arguments(self):
        fixture = (ROOT / "test/fixtures/text-contains-order.lan").read_text().replace(
            "Text.contains", f"Text.{OPERATION}")
            ...
            with self.subTest(unused=unused):
                ...
                if unused:
                    omitted = program.read_text().replace(
                        f"  let same : Bool := Text.{OPERATION} Bytes (left db) (right db) in\n", "")
    ...
    def test_refusal_before_output(self):
        ...
        for expression in [f"Text.{OPERATION} Nat 1 1", f'Text.{OPERATION} Bytes b"x" 1']:
    ...
    def test_native(self):
        ...
        print(f"LAN-TEXT-BOUNDARIES NATIVE OK operation={OPERATION} observations={len(expected_native())}")

    def test_uri_routing(self):
        for uri, expected in [("/static/main.css", "stylesheet"), ("/static/main.js", "asset"),
                              ("/else/static/main.css", "other"), ("/static/main.css/", "asset"),
                              ("/static/main.css?v=1", "stylesheet"), ("/static/", "asset")]:
            with self.subTest(uri=uri):
                actual = run(DRIVER, "run", "--request", uri, ROOT / "test/fixtures/text-boundaries.lan")
                self.assertEqual((actual.returncode, actual.stdout, actual.stderr),
                                 (0, response(expected), b""))
...
def mutations():
    (ROOT / ".gatework").mkdir(exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="lanyard-text-boundaries-mut-", dir=ROOT / ".gatework") as directory:
        work = Path(directory)
        emitted = emit(work)
        for name, before, after in [("clean", "", ""), *mutation_cases(), ("restored", "", "")]:
            if before and emitted.count(before) != 1:
                sys.exit(f"LAN-TEXT-BOUNDARIES-MUT FAIL anchor {name}: {emitted.count(before)}")
            ...
            if actual.returncode != 0 or actual.stderr:
                sys.exit(f"LAN-TEXT-BOUNDARIES-MUT FAIL runtime {name}: {actual.stderr!r}")
            ...
            if observed_names != [row.rsplit(" ", 1)[0] for row in expected]:
                sys.exit(f"LAN-TEXT-BOUNDARIES-MUT FAIL incomplete observations {name}")
            if before:
                failed = sum(row.endswith(" false") for row in rows)
                if not failed:
                    sys.exit(f"LAN-TEXT-BOUNDARIES-MUT SURVIVED {name}")
                print(f"LAN-TEXT-BOUNDARIES-MUT KILLED {name} observations={failed}")
            elif rows != expected:
                sys.exit(f"LAN-TEXT-BOUNDARIES-MUT FAIL {name}")
            else:
                print(f"LAN-TEXT-BOUNDARIES-MUT OK {name}")
...
def leaked_directories(started):
    """Temp directories with this file's prefixes that were created after the run started."""
    places = [(ROOT / ".gatework", "text-boundaries-"),
              (ROOT / ".gatework", "lanyard-text-boundaries-mut-")]
    return [entry for place, prefix in places if place.is_dir() for entry in place.iterdir()
            if entry.is_dir() and entry.name.startswith(prefix) and created_at(entry) >= started]


def relaunch(operation, started, row):
    """Run this file for one operation; a wrapper timeout removes this run's leaked directories."""
    command = [sys.executable, "-P", __file__, "--operation", operation, *sys.argv[1:]]
    with contextlib.suppress(subprocess.TimeoutExpired):
        return subprocess.run(command, cwd=ROOT, timeout=CHILD_BUDGET).returncode
    # Only a suppressed TimeoutExpired reaches this row: the child was killed.
    leaked = leaked_directories(started)
    for entry in leaked:
        shutil.rmtree(entry, ignore_errors=True)
    remaining = [entry for entry in leaked if entry.exists()]
    sys.exit(f"{row} FAIL timeout {operation}: no exit within {CHILD_BUDGET} s, "
             f"removed {len(leaked) - len(remaining)} of {len(leaked)} leaked directories")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--mutations", action="store_true")
    parser.add_argument("--operation", choices=["starts_with", "ends_with"])
    args, remaining = parser.parse_known_args()
    if args.operation is None:
        started = time.time()
        row = "LAN-TEXT-BOUNDARIES-MUT" if args.mutations else "LAN-TEXT-BOUNDARIES"
        for operation in ["starts_with", "ends_with"]:
            returncode = relaunch(operation, started, row)
            if returncode:
                sys.exit(returncode)
        sys.exit(0)
    OPERATION = args.operation
    BASE = BASE.replace("Text.contains", f"Text.{OPERATION}")
    print(f"LAN-TEXT-BOUNDARIES operation={OPERATION}", flush=True)
    if args.mutations:
        if remaining:
