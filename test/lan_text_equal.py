"""Check text comparison, Boolean branching, failures and native semantics."""
import argparse
import itertools
...
MUTANTS = [
    ("inverted", "__lan_left == __lan_right", "__lan_left != __lan_right"),
    ("length", "__lan_left == __lan_right", "__lan_left.len() == __lan_right.len()"),
    ("prefix", "__lan_left == __lan_right", "__lan_left.starts_with(&__lan_right)"),
    ("trimmed", "__lan_left == __lan_right", "__lan_left.trim() == __lan_right.trim()"),
    ("unchecked_right", "let __lan_right = lan_model_text_to_4279746573(&__lan_right_arg)?; if __lan_left ==",
     "let __lan_right = lan_model_text_to_4279746573(&__lan_right_arg).unwrap_or_default(); if __lan_left =="),
]
...
def require(result):
    if result.returncode != 0 or result.stderr:
        sys.exit(f"LAN-TEXT-EQUAL FAIL: {result.stdout!r} {result.stderr!r}")
...
def native_harness():
    pairs = ",\n".join(f"({rust_string(left)}, {rust_string(right)}, "
                       f"{rust_string('yes' if left == right else 'no')})" for left, right in PAIRS)
...
def compile_native(work, emitted, name):
    ...
    if result.returncode != 0:
        sys.exit(f"LAN-TEXT-EQUAL FAIL native build: {result.stdout!r} {result.stderr!r}")
    return run(binary)


class TextEqual(unittest.TestCase):
    def setUp(self):
        (ROOT / ".gatework").mkdir(exist_ok=True)
        directory = tempfile.TemporaryDirectory(prefix="text-equal-", dir=ROOT / ".gatework")
        ...
        for action in ["save", "Save", "save ", "", "saved", "save\0", "保存"]:
            with self.subTest(action=action):
                actual = run(DRIVER, "run", "--request", "/", "--form", urlencode({"action": action}),
                             ROOT / "test/fixtures/text-equal.lan")
                self.assertEqual((actual.returncode, actual.stdout, actual.stderr),
                                 (0, response("saved" if action == "save" else "unknown action"), b""))

    def test_missing_form_field(self):
        actual = run(DRIVER, "run", "--request", "/", "--form", "", ROOT / "test/fixtures/text-equal.lan")
    ...
    def test_interpreter_pairs(self):
        ...
        for left, right in PAIRS:
            with self.subTest(left=left, right=right):
                ...
                self.assertEqual((actual.returncode, actual.stdout, actual.stderr),
                                 (0, response("yes" if left == right else "no"), b""))
    ...
    def test_effectful_arguments(self):
        fixture = (ROOT / "test/fixtures/text-equal-order.lan").read_text()
    ...
    def test_refusal_before_output(self):
        ...
        for expression in ["Text.equal Nat 1 1", 'Text.equal Bytes b"x" 1']:
    ...
    def test_native(self):
        ...
        print(f"LAN-TEXT-EQUAL NATIVE OK observations={len(expected_native())}")


def mutations():
    with tempfile.TemporaryDirectory(prefix="lanyard-text-equal-mut-") as directory:
        work = Path(directory)
        emitted = emit(work)
        for name, before, after in [("clean", "", ""), *MUTANTS, ("restored", "", "")]:
            if before and emitted.count(before) != 1:
                sys.exit(f"LAN-TEXT-EQUAL-MUT FAIL anchor {name}: {emitted.count(before)}")
            ...
            if actual.returncode != 0 or actual.stderr:
                sys.exit(f"LAN-TEXT-EQUAL-MUT FAIL runtime {name}: {actual.stderr!r}")
            ...
            if observed_names != [row.rsplit(" ", 1)[0] for row in expected]:
                sys.exit(f"LAN-TEXT-EQUAL-MUT FAIL incomplete observations {name}")
            if before:
                failed = sum(row.endswith(" false") for row in rows)
                if not failed:
                    sys.exit(f"LAN-TEXT-EQUAL-MUT SURVIVED {name}")
                print(f"LAN-TEXT-EQUAL-MUT KILLED {name} observations={failed}")
            elif rows != expected:
                sys.exit(f"LAN-TEXT-EQUAL-MUT FAIL {name}")
            else:
                print(f"LAN-TEXT-EQUAL-MUT OK {name}")
