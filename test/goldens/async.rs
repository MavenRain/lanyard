...
async fn f_70757368(a0: Arc<toasty::Db>) -> Result<(), Error> {
    Ok({ let __lan_arg_0 = Arc::clone(&(a0)); (&*(__lan_arg_0)).push_schema().await? })
}

async fn f_666f7277617264(a0: Arc<toasty::Db>) -> Result<(), Error> {
    f_70757368(Arc::clone(&(a0))).await
}

async fn f_7477696365(a0: Arc<toasty::Db>) -> Result<(), Error> {
    Ok({ let v1 = f_666f7277617264(Arc::clone(&(a0))).await?; f_70757368(Arc::clone(&(a0))).await? })
}

async fn f_73657175656e7469616c(a0: Arc<toasty::Db>, a1: Arc<toasty::Db>) -> Result<(), Error> {
    Ok({ let v2 = f_70757368(Arc::clone(&(a0))).await?; f_70757368(Arc::clone(&(a1))).await? })
}

async fn f_6b656570(a0: Arc<toasty::Db>, a1: Arc<topcoat::router::Uri>) -> Result<topcoat::router::error::SeeOther, Error> {
    Ok({ let v2 = f_70757368(Arc::clone(&(a0))).await?; { let __lan_arg_0 = Arc::clone(&(a1)); topcoat::router::error::see_other((&*(__lan_arg_0)).to_string()) } })
}

async fn f_63686f6f7365(a0: Arc<T73756d28756e69742c756e697429>, a1: Arc<toasty::Db>) -> Result<(), Error> {
    Ok(match (*(Arc::clone(&(a0)))).clone() { T73756d28756e69742c756e697429::V0(v2) => f_70757368(Arc::clone(&(a1))).await?, T73756d28756e69742c756e697429::V1(v2) => () })
}

fn f_70757265(a0: Arc<Nat>) -> Result<Nat, Error> {
